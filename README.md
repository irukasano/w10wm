...
その上で、モニターごとにウィンドウの整理方法を設定でき、整理された中でアクティヴウィンドウを上下移動したり、
またはフォーカスを上下移動するアプリケーションを作ることにしました。
...
という逆に甘えた気持ちで OSS ライセンスにしています。
自由な改変をしていただいていいのですが、どう改変したのか後学のために知りたい（こともある気がする）ので GPL v2.0 にしました。
...
* 標準ショートカットキーのうち、いらないでしょ、と私がおもったものは本アプリケーションで使ったりしています。（たとえばコルタナさんが起動するWin+Cとか）

## 前提

* 起動すると無言でタスクトレイに居座ります
...
* だいぶ減りましたが、例外が発生するとダイアログなどなしに無言でいなくなります（Win+jとかが効かなくなるので気づきます）
* バグはあるはず
* .Net Framework 4.8 で作っています。Windows　のバージョンが Windows 10 May 2019 Update あたり以降であればおそらくインストール済です。
...
* モニターハイライト機能の動きがあやしいのを直したい（連打すると特に怪しい）
* 通常時はログの出力を少し減らしたい
