...
namespace windows10windowManagerUtil.Util
{
    public class Logger
    {
         ...
         * </summary>
         */
        public static void DebugWindowInfo(string message, IWindowInfoWithHandle windowInfoWithHandle)
        {
            var windowHandle = windowInfoWithHandle.windowHandle;
            var windowTitle = windowInfoWithHandle.windowTitle;
            var monitorHandle = windowInfoWithHandle.monitorHandle;
            var m = message + $" : {windowTitle} ( hWnd={windowHandle}, hMonitor={monitorHandle} )";
            Logger.WriteLine(m);
        }

        public static void DebugMonitor(string message, IMonitorInfoWithHandle monitorInfoWithHandle)
        {
